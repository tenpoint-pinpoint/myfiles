...
p1 <- ggplot(price4) +
  geom_line(mapping = aes(x = times, y= x5202)) +
  labs(y="板硝子x5202") +
  theme_gray(base_family = "HiraKakuPro-W3")
p2 <- ggplot(price4) +
  geom_line(mapping = aes(x = times, y= x7272)) +
  labs(y="ヤマハx7272")+
  theme_gray(base_family = "HiraKakuPro-W3")
p3 <- ggplot(price4) +
  geom_line(mapping = aes(x = times, y= x4927)) +
  labs(y="ポーラx4927")+
  theme_gray(base_family = "HiraKakuPro-W3")
p4 <- ggplot(price4) +
  geom_line(mapping = aes(x = times, y= x4502)) +
  labs(y="武田薬品x4502")+
  theme_gray(base_family = "HiraKakuPro-W3")
...
#このままだと、上昇比率はわかるものの日々の価格変動のしやすさまでは読み取れない
#縦軸のメモリが変わっただけ
#ここで収益率を使う。収益率は日々の価格変動幅を前日の価格で基準化したもの
#数式は   r(t)=logP(t)-log(t-1)  ※対数差収益率をいう
